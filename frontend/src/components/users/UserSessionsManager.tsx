...
export const UserSessionsManager: React.FC<UserSessionsManagerProps> = ({ 
  ...
  const activeSessions = sessions.filter(s => s.isActive && !isSessionExpired(s.expiresAt));
  const inactiveSessions = sessions.filter(s => !s.isActive || isSessionExpired(s.expiresAt));
