...
const VALID_SVG =
  '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100"><rect width="100" height="100" fill="red"/><circle cx="50" cy="50" r="30" fill="blue"/></svg>';
...
describe('useSVGUpload', () => {
  ...
  describe('validateSVG', () => {
    ...
    it('rejects non-SVG extension', () => {
      const { result } = renderHook(() => useSVGUpload());
      const file = new File(['x'.repeat(200)], 'image.png', {
        type: 'image/png',
      });
  ...
  describe('uploadSVG', () => {
    it('returns upload result on valid SVG', async () => {
      ...
      let uploadResult:
        | Awaited<ReturnType<typeof result.current.uploadSVG>>
        | undefined;
      await act(async () => {
    ...
    it('calls mutation with correct variables', async () => {
      const { result } = renderHook(() => useSVGUpload());
      await act(async () => {
        await result.current.uploadSVG(makeSVGFile());
      });
    ...
    it('throws and sets error when server responds success=false', async () => {
      mockUploadSvgMutate.mockResolvedValueOnce({
        data: {
          uploadSvg: { success: false, message: 'Error servidor', data: null },
        },
      });
      const { result } = renderHook(() => useSVGUpload());
      ...
      await act(async () => {
        await expect(
          result.current.uploadSVG(makeSVGFile())
        ).rejects.toBeDefined();
      });
