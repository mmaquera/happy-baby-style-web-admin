...
export const useTags = () => {
  ...
  const createTag = useCallback(
    async (
      tagName: string,
      metadata: Omit<TagMetadata, 'usageCount' | 'createdAt'>
    ) => {
      setIsLoading(true);
      setError(null);
  ...
  const updateTag = useCallback(
    async (tagName: string, updates: Partial<TagMetadata>) => {
      setIsLoading(true);
      setError(null);
  ...
  const getTagSuggestions = useCallback(
    (context: string, limit: number = 3) => {
      ...
      if (context.includes('edad') || context.includes('meses')) {
        ...
        Object.entries(ageTags)
          .slice(0, limit)
          .forEach(([tagName, metadata]) => {
