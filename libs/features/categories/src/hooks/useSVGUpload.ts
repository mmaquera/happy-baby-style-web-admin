...
import { useUploadSvgMutation } from '@happy-baby/infrastructure-graphql';
import {
...
export const useSVGUpload = (): UseSVGUploadReturn => {
  ...
  const validateSVG = useCallback((file: File): SVGValidationResult => {
    ...
    if (!hasValidExtension) {
      errors.push(
        `Extensión de archivo no válida. Solo se permiten archivos .svg`
      );
    }
    ...
    if (file.size > SVG_VALIDATION_RULES.maxSize) {
      errors.push(
        `El archivo es demasiado grande. Máximo ${SVG_VALIDATION_RULES.maxSize / 1000}KB.`
      );
    }

    if (file.size < SVG_VALIDATION_RULES.minSize) {
      errors.push(
        `El archivo es demasiado pequeño. Mínimo ${SVG_VALIDATION_RULES.minSize} bytes.`
      );
    }
    ...
    if (file.size > 1000000) {
      // 1MB
      warnings.push(
        'Archivo SVG grande detectado. Se recomienda optimizar el archivo.'
      );
    }
  ...
  const validateSVGContent = useCallback(
    async (file: File): Promise<{ isValid: boolean; errors: string[] }> => {
      try {
  ...
  const uploadSVG = useCallback(
    async (file: File): Promise<SVGUploadResult> => {
      try {
        setLoading(true);
        setError(null);
        setProgress({
        ...
        if (!contentValidation.isValid) {
          throw new Error(
            `Contenido SVG inválido: ${contentValidation.errors.join(', ')}`
          );
        }
        ...
        if (result.data?.uploadSvg?.success && result.data.uploadSvg.data) {
          ...
          const errorMessage =
            result.data?.uploadSvg?.message || 'Error desconocido en la subida';
