...
export const useUserProfile = ({
  ...
  const [createAddressMutation, { loading: creatingAddress }] =
    useCreateUserAddressMutation({
      refetchQueries: [GetUserProfileDocument],
      awaitRefetchQueries: true,
    });
  ...
  const [updateAddressMutation, { loading: updatingAddress }] =
    useUpdateUserAddressMutation({
      refetchQueries: [GetUserProfileDocument],
      awaitRefetchQueries: true,
    });
  ...
  const [deleteAddressMutation, { loading: deletingAddress }] =
    useDeleteUserAddressMutation({
      refetchQueries: [GetUserProfileDocument],
      awaitRefetchQueries: true,
    });
  ...
  const [setDefaultAddressMutation, { loading: settingDefault }] =
    useSetDefaultAddressMutation({
      refetchQueries: [GetUserProfileDocument],
      awaitRefetchQueries: true,
    });
  ...
  const createAddress = useCallback(
    async (input: CreateUserAddressInput) => {
      try {
        ...
        if (result.data?.createUserAddress?.success) {
          ...
          const errorMessage =
            result.data?.createUserAddress?.message ||
            'Error al crear dirección';
          throw new Error(errorMessage);
        }
      } catch (error) {
        const errorMessage =
          error instanceof Error ? error.message : 'Error al crear dirección';
        toast.error(errorMessage);
        throw error;
      }
    },
  ...
  const updateAddress = useCallback(
    async (addressId: string, input: UpdateUserAddressInput) => {
      try {
        ...
        if (result.data?.updateUserAddress?.success) {
          ...
          const errorMessage =
            result.data?.updateUserAddress?.message ||
    ...
    async (addressId: string) => {
      try {
        ...
        if (result.data?.deleteUserAddress?.success) {
          ...
          const errorMessage =
            result.data?.deleteUserAddress?.message ||
            'Error al crear dirección';
          throw new Error(errorMessage);
        }
      } catch (error) {
    ...
    async (addressId: string) => {
      try {
        ...
        if (result.data?.setDefaultAddress?.success) {
          ...
          const errorMessage =
            result.data?.setDefaultAddress?.message ||
