...
const GetProductByIdUseCase_1 = require("@application/use-cases/product/GetProductByIdUseCase");
...
describe('GetProductByIdUseCase', () => {
    let useCase;
    beforeEach(() => {
        useCase = new GetProductByIdUseCase_1.GetProductByIdUseCase(mockProductRepository);
        jest.clearAllMocks();
    });
    describe('execute', () => {
        it('should return product when valid ID is provided', async () => {
            // Arrange
            const mockProduct = new Product_1.ProductEntity('1', 'category-1', 'Test Product', 'Test Description', 29.99, undefined, 'SKU-1', [], {}, true, 10, ['test'], 4.5, 5, new Date(), new Date());
            const request = { id: '1' };
