...
export const UploadContainer: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  children,
  className,
  ...rest
}) => (
...
> = ({ progress, className, style, ...rest }) => (
  <div
    className={cn('h-full bg-brand-purple transition-[width] duration-300', className)}
...
> = ({ className, ...rest }) => (
  <img
    className={cn('h-full w-full bg-muted object-contain object-center', className)}
    {...rest}
  />
);
...
> = ({ className, ...rest }) => (
  <div
    className={cn('h-5 w-5 animate-spin rounded-full border-2 border-muted border-t-brand-purple', className)}
    {...rest}
  />
);
