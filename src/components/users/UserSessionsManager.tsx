...
const isSuspicious = (session: UserSession) =>
  ...
  (!!session.userAgent && (session.userAgent.includes('bot') || session.userAgent.includes('crawler')));
...
const SecurityAlert = ({
  ...
  <div className={cn('mt-4 flex items-center gap-3 rounded-lg p-4 text-sm', ALERT_CLASS[type])}>
    {icon}
    <div>{children}</div>
  </div>
);
  ...
  if (sessions.length === 0) {
    return (
      <div className='rounded-xl border-2 border-dashed border-border bg-muted py-12 text-center text-muted-foreground'>
        ...
        <h3 className='mt-4 mb-2 font-heading text-xl text-foreground'>Sin sesiones activas</h3>
        <p className='text-base text-muted-foreground'>Este usuario no tiene sesiones registradas</p>
      </div>
    );
  }
      ...
      >
              ...
              <div className='text-sm text-muted-foreground'>
                {isActive ? (
                  <>
                    Sesión activa
                    {suspicious ? (
                      <span className='ml-1 font-medium text-yellow-600'>⚠️ Sospechosa</span>
                    ) : null}
                  </>
                ) : expired ? (
                  'Sesión expirada'
        ...
        <div className='mt-4 grid gap-4 border-t border-border pt-4 [grid-template-columns:repeat(auto-fit,minmax(180px,1fr))]'>
          <DetailItem label='Estado'>
            ...
            >
              {isActive ? <CheckCircleIcon size={12} /> : <AlertCircleIcon size={12} />}
    ...
    <div className='flex flex-col gap-3'>
      ...
      <div className='mb-6 rounded-xl border border-border bg-white p-6 shadow-sm'>
        ...
        <div className='grid gap-4 [grid-template-columns:repeat(auto-fit,minmax(120px,1fr))]'>
          {[
            { value: securityAnalysis.totalSessions, label: 'Total Sesiones', color: '' },
            { value: securityAnalysis.activeSessions, label: 'Activas', color: 'text-green-600' },
            { value: securityAnalysis.expiredSessions, label: 'Expiradas', color: 'text-yellow-600' },
            { value: securityAnalysis.suspiciousSessions, label: 'Sospechosas', color: 'text-destructive' },
          ].map(({ value, label, color }) => (
            <div
              key={label}
              className='rounded-lg border border-border bg-muted p-3 text-center transition-colors hover:bg-card'
            >
              <div className={cn('mb-2 text-2xl font-bold text-foreground', color)}>
                {value}
              </div>
        ...
        {securityAnalysis.multipleLocations ? (
          <SecurityAlert type='info' icon={<GlobeIcon size={16} />}>
            <strong>🌍 Múltiples Ubicaciones:</strong> El usuario tiene sesiones activas desde{' '}
            {securityAnalysis.uniqueIPs} ubicación(es) diferente(s).
            {securityAnalysis.uniqueIPs > 3 ? ' Esto podría indicar un uso compartido de la cuenta.' : null}
          </SecurityAlert>
        ) : null}
        ...
        {!securityAnalysis.hasSecurityIssues && securityAnalysis.activeSessions > 0 ? (
          <SecurityAlert type='success' icon={<CheckCircleIcon size={16} />}>
            <strong>✅ Estado de Seguridad Óptimo:</strong> Todas las sesiones activas parecen ser legítimas y seguras.
          </SecurityAlert>
        ) : null}
      </div>
      ...
      {activeSessions.length > 0 ? (
        <div>
          ...
          {activeSessions.map(s => <SessionCard key={s.id} session={s} isActive={true} />)}
        </div>
      ) : null}
      ...
      {inactiveSessions.length > 0 ? (
        <div className='mt-6'>
          ...
          {inactiveSessions.map(s => <SessionCard key={s.id} session={s} isActive={false} />)}
        </div>
      ) : null}
    </div>
  );
