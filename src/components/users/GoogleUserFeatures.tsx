...
export const GoogleUserFeatures: React.FC<GoogleUserFeaturesProps> = ({
  ...
  const getGoogleSyncStatus = (): {
    ...
    if (!googleAccount.expiresAt)
      return { status: 'success', text: 'Sin expiración', icon: <CheckCircleIcon size={12} /> };
    if (isTokenExpired(googleAccount.expiresAt))
      return { status: 'error', text: 'Token expirado', icon: <XCircleIcon size={12} /> };
    const hours = Math.floor(
      (new Date(googleAccount.expiresAt).getTime() - Date.now()) / 3_600_000
    );
    if (hours < 24)
      return { status: 'warning', text: `Expira en ${hours}h`, icon: <AlertTriangleIcon size={12} /> };
    return { status: 'success', text: 'Sincronizado', icon: <CheckCircleIcon size={12} /> };
  };
    ...
    <div className='flex flex-col gap-4'>
      ...
      <Card className='border-l-4 p-4' style={{ borderLeftColor: '#4285f4' }}>
        ...
        <div className='mb-3 grid gap-3 [grid-template-columns:repeat(auto-fit,minmax(200px,1fr))]'>
          ...
          <InfoRow label='Email Verificado'>
            {user.emailVerified ? (
              <span className={cn('inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-medium uppercase tracking-[0.5px]', STATUS_CLASS.success)}>
                <CheckCircleIcon size={12} /> Verificado por Google
              </span>
            ) : (
              <span className={cn('inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-medium uppercase tracking-[0.5px]', STATUS_CLASS.error)}>
      ...
      <Card className='border-l-4 p-4' style={{ borderLeftColor: '#4285f4' }}>
        ...
        <div className='mb-3 rounded-md bg-muted p-3'>
          ...
          <ul className='m-0 list-none p-0'>
            {SECURITY_FEATURES.map(f => (
              <li key={f} className='flex items-center gap-2 py-1 text-sm text-muted-foreground'>
        ...
        <div className='mb-3 rounded-md bg-muted p-3'>
          ...
          <ul className='m-0 list-none p-0'>
            {SYNCED_DATA.map(d => (
              <li key={d} className='flex items-center gap-2 py-1 text-sm text-muted-foreground'>
