import { createUserSchema, updateUserSchema, createUserFormSchema } from '../userSchema';
...
describe('createUserSchema', () => {
  ...
  it('rejects invalid email', () => {
    const result = createUserSchema.safeParse({ ...validData, email: 'bad-email' });
  ...
  it('rejects password shorter than 8 characters', () => {
    const result = createUserSchema.safeParse({ ...validData, password: 'short1' });
    expect(result.success).toBe(false);
  });
  ...
  it('accepts all valid roles', () => {
    ...
    for (const role of roles) {
      expect(createUserSchema.safeParse({ ...validData, role }).success).toBe(true);
    }
  });
});
...
describe('createUserFormSchema', () => {
  ...
  it('rejects invalid email format', () => {
    const result = createUserFormSchema.safeParse({ ...validData, email: 'not-an-email' });
    expect(result.success).toBe(false);
  });

  it('rejects empty password', () => {
    const result = createUserFormSchema.safeParse({ ...validData, password: '' });
    expect(result.success).toBe(false);
    expect(result.error?.flatten().fieldErrors.password).toBeDefined();
  });

  it('rejects password shorter than 8 characters', () => {
    const result = createUserFormSchema.safeParse({ ...validData, password: 'short1!' });
    expect(result.success).toBe(false);
  });

  it('rejects empty firstName', () => {
    const result = createUserFormSchema.safeParse({ ...validData, firstName: '' });
    expect(result.success).toBe(false);
  });

  it('rejects empty lastName', () => {
    const result = createUserFormSchema.safeParse({ ...validData, lastName: '' });
    expect(result.success).toBe(false);
  });

  it('rejects future dateOfBirth', () => {
    const result = createUserFormSchema.safeParse({ ...validData, dateOfBirth: '2099-01-01' });
    expect(result.success).toBe(false);
    expect(result.error?.flatten().fieldErrors.dateOfBirth).toBeDefined();
  });

  it('rejects dateOfBirth before 1900', () => {
    const result = createUserFormSchema.safeParse({ ...validData, dateOfBirth: '1899-12-31' });
    expect(result.success).toBe(false);
  });

  it('rejects invalid dateOfBirth format', () => {
    const result = createUserFormSchema.safeParse({ ...validData, dateOfBirth: 'not-a-date' });
    expect(result.success).toBe(false);
  });

  it('accepts null dateOfBirth', () => {
    const result = createUserFormSchema.safeParse({ ...validData, dateOfBirth: null });
    expect(result.success).toBe(true);
  });

  it('rejects invalid role', () => {
    const result = createUserFormSchema.safeParse({ ...validData, role: 'superadmin' as never });
    expect(result.success).toBe(false);
  });
