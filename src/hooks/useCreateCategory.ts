import { useCreateCategoryMutation, useUploadSvgMutation } from '@/generated/graphql';
import { CreateCategoryInput } from '@/generated/graphql';
import { toast } from 'react-hot-toast';
import { useState, useCallback } from 'react';

interface UseCreateCategoryReturn {
  create: (input: CreateCategoryInput) => Promise<any>;
...
export const useCreateCategory = (): UseCreateCategoryReturn => {
  const [createCategoryMutation, { loading: creating, error: createError }] = useCreateCategoryMutation({
  ...
  const validateCategoryInput = useCallback((input: CreateCategoryInput): string[] => {
    const errors: string[] = [];
  ...
  const uploadSVG = useCallback(async (file: File, categoryId?: string): Promise<string | null> => {
    try {
      ...
      if (!response.success) {
        const errorMessage = response.message || 'Error al subir el archivo SVG';
        ...
        if (errorMessage.includes('Invalid file type') || errorMessage.includes('Only JPEG, PNG and WEBP')) {
          const specificError = 'El backend no acepta archivos SVG. Por favor contacta al administrador para habilitar el soporte de SVG.';
      ...
      const errorMessage = error instanceof Error ? error.message : 'Error inesperado al subir SVG';
      setLocalError(errorMessage);
      toast.error(errorMessage);
      return null;
    }
...
const validateSVGFile = (file: File): string[] => {
  ...
  if (!hasValidMimeType) {
    errors.push(`Tipo MIME inesperado: ${file.type}. Verificando contenido del archivo...`);
  }
