...
vi.mock('@/generated/graphql', () => ({
  useSetUserPasswordMutation: () => [mockMutate, { loading: false, error: null }],
}));
...
describe('useSetUserPassword', () => {
  beforeEach(() => {
    ...
    mockMutate.mockResolvedValue({
      data: { setUserPassword: { success: true, message: 'Contraseña establecida', code: 'OK' } },
    });
  });
  ...
  it('returns false and shows error when not authenticated', async () => {
    ...
    expect(toast.error).toHaveBeenCalledWith('Debes estar autenticado para realizar esta acción');
  });
  ...
  describe('password validation', () => {
    ...
    it('rejects short password (< 8 chars)', async () => {
      const { result } = renderHook(() => useSetUserPassword());
      await act(async () => { await result.current.setUserPassword('user-1', 'Ab1!'); });
      expect(result.current.error).toContain('8 caracteres');
    });

    it('rejects password without uppercase', async () => {
      const { result } = renderHook(() => useSetUserPassword());
      await act(async () => { await result.current.setUserPassword('user-1', 'password1!'); });
    ...
    it('rejects password without number', async () => {
      const { result } = renderHook(() => useSetUserPassword());
      await act(async () => { await result.current.setUserPassword('user-1', 'Password!'); });
  ...
  it('returns true and shows success toast on success', async () => {
    ...
    let success: boolean | undefined;
    await act(async () => { success = await result.current.setUserPassword('user-1', STRONG_PASSWORD); });
  ...
  it('returns false when server responds with success=false', async () => {
    mockMutate.mockResolvedValueOnce({
      data: { setUserPassword: { success: false, message: 'User not found', code: 'NOT_FOUND' } },
    });
    const { result } = renderHook(() => useSetUserPassword());
  ...
  it('clearError resets error state', async () => {
    const { result } = renderHook(() => useSetUserPassword());
    await act(async () => { await result.current.setUserPassword('user-1', 'bad'); });
