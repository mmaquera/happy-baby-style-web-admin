...
const makeFileList = (names: string[]): FileList => {
  const files = names.map(name => new File(['content'], name, { type: 'image/jpeg' }));
  const fileList = {
    length: files.length,
...
describe('useImageUpload', () => {
  ...
  describe('upload', () => {
    ...
    it('calls mutation with entityId and entityType when provided', async () => {
      ...
      await act(async () => {
        await result.current.upload(makeFileList(['img.jpg']), 'product-1', 'product');
      });

      expect(mockUploadMutate).toHaveBeenCalledWith(
        expect.objectContaining({
          variables: expect.objectContaining({ entityId: 'product-1', entityType: 'product' }),
        })
      );
    });
    ...
    it('returns failure result when server responds success=false', async () => {
      mockUploadMutate.mockResolvedValue({
        data: {
          uploadImage: { success: false, message: 'Tipo no permitido', data: null },
        },
      });
      const { result } = renderHook(() => useImageUpload());
    ...
    it('handles multiple files — partial success', async () => {
      ...
      let uploadResult: Awaited<ReturnType<typeof result.current.upload>> | undefined;
      await act(async () => {
        uploadResult = await result.current.upload(makeFileList(['a.jpg', 'b.jpg']));
      });
  ...
  it('clearError resets error state', async () => {
    ...
    await act(async () => { await result.current.upload(makeFileList(['img.jpg'])); });
