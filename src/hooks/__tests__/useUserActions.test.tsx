...
const makeUser = (id = 'user-1', isActive = true): User => ({
  ...
  emailVerified: true,
  lastLoginAt: null,
  profile: { id, firstName: 'Juan', lastName: 'García', phone: null, dateOfBirth: null, avatar: null, fullName: 'Juan García' },
  addresses: [],
...
describe('useUserActions', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockListExecute.mockResolvedValue(ok({ items: [], total: 0, hasMore: false }));
  ...
  describe('loadUsers', () => {
    it('loads users and updates state', async () => {
      const user = makeUser();
      mockListExecute.mockResolvedValueOnce(ok({ items: [user], total: 1, hasMore: false }));
      const { result } = renderHook(() => useUserActions());
    ...
    it('sets error on failure', async () => {
      mockListExecute.mockResolvedValueOnce(err(new DomainError('No se pudo cargar', 'ERR')));
      const { result } = renderHook(() => useUserActions());
  ...
  describe('createUser', () => {
    it('prepends created user to list', async () => {
      ...
      mockListExecute.mockResolvedValueOnce(ok({ items: [existing], total: 1, hasMore: false }));
      const newUser = makeUser('new-user');
      mockCreateExecute.mockResolvedValueOnce(ok(newUser));

      const { result } = renderHook(() => useUserActions());
      await act(async () => { await result.current.loadUsers(); });
      await act(async () => { await result.current.createUser({ email: 'new@example.com', firstName: 'Ana', lastName: 'López' }); });
    ...
    it('shows success toast', async () => {
      const { result } = renderHook(() => useUserActions());
      await act(async () => { await result.current.createUser({ email: 'x@example.com', firstName: 'A', lastName: 'B' }); });
      expect(toast.success).toHaveBeenCalledWith('Usuario creado exitosamente');
    });

    it('shows error toast on failure', async () => {
      mockCreateExecute.mockResolvedValueOnce(err(new DomainError('Email duplicado', 'DUPLICATE')));
      const { result } = renderHook(() => useUserActions());

      await act(async () => {
        const success = await result.current.createUser({ email: 'dup@example.com', firstName: 'A', lastName: 'B' });
  ...
  describe('deleteUser', () => {
    it('removes user from list on success', async () => {
      ...
      mockListExecute.mockResolvedValueOnce(ok({ items: [u1, u2], total: 2, hasMore: false }));

      const { result } = renderHook(() => useUserActions());
      await act(async () => { await result.current.loadUsers(); });
      await act(async () => { await result.current.deleteUser('u-1'); });
  ...
  describe('activateUser', () => {
    it('updates user to active in list', async () => {
      ...
      mockListExecute.mockResolvedValueOnce(ok({ items: [inactive], total: 1, hasMore: false }));

      const { result } = renderHook(() => useUserActions());
      await act(async () => { await result.current.loadUsers(); });
      await act(async () => { await result.current.activateUser('u-1'); });
    ...
    it('returns false and shows error on failure', async () => {
      mockActivateExecute.mockResolvedValueOnce(err(new DomainError('No existe', 'NOT_FOUND')));
      const { result } = renderHook(() => useUserActions());
  ...
  describe('deactivateUser', () => {
    it('updates user to inactive in list', async () => {
      ...
      mockListExecute.mockResolvedValueOnce(ok({ items: [active], total: 1, hasMore: false }));

      const { result } = renderHook(() => useUserActions());
      await act(async () => { await result.current.loadUsers(); });
      await act(async () => { await result.current.deactivateUser('u-1'); });
