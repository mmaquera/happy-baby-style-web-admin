...
const makeOrder = (id = 'order-1', status: Order['status'] = 'pending'): Order => ({
  id,
  orderNumber: `ORD-${id}`,
  status,
  subtotal: 100,
...
describe('useOrderActions', () => {
  beforeEach(() => {
    ...
    mockUpdateStatusExecute.mockResolvedValue(ok(makeOrder('order-1', 'confirmed')));
  ...
  describe('loadOrders', () => {
    it('loads orders and updates state', async () => {
      ...
      mockListExecute.mockResolvedValueOnce(ok({ items: [order], total: 1, hasMore: false }));
      const { result } = renderHook(() => useOrderActions());
    ...
    it('sets error on failure', async () => {
      mockListExecute.mockResolvedValueOnce(err(new DomainError('Error carga', 'ERR')));
      const { result } = renderHook(() => useOrderActions());
  ...
  describe('clearError', () => {
    it('clears error state', async () => {
      mockListExecute.mockResolvedValueOnce(err(new DomainError('fail', 'ERR')));
      const { result } = renderHook(() => useOrderActions());
  ...
  describe('updateStatus', () => {
    it('updates order status in list', async () => {
      ...
      const { result } = renderHook(() => useOrderActions());
      await act(async () => { await result.current.loadOrders(); });
      await act(async () => { await result.current.updateStatus('order-1', 'confirmed'); });
    ...
    it('shows success toast', async () => {
      ...
      await act(async () => {
        const success = await result.current.updateStatus('order-1', 'confirmed');
        expect(success).toBe(true);
      });

      expect(toast.success).toHaveBeenCalledWith('Estado del pedido actualizado');
    });

    it('shows error toast on failure', async () => {
      mockUpdateStatusExecute.mockResolvedValueOnce(err(new DomainError('Error estado', 'ERR')));
      const { result } = renderHook(() => useOrderActions());
  ...
  describe('cancelOrder', () => {
    it('updates order to cancelled status', async () => {
      ...
      const { result } = renderHook(() => useOrderActions());
      await act(async () => { await result.current.loadOrders(); });
      await act(async () => { await result.current.cancelOrder('order-1'); });
    ...
    it('shows success toast', async () => {
      ...
      expect(toast.success).toHaveBeenCalledWith('Pedido cancelado exitosamente');
    });

    it('shows error toast on failure', async () => {
      mockCancelExecute.mockResolvedValueOnce(err(new DomainError('No se puede cancelar', 'ERR')));
      const { result } = renderHook(() => useOrderActions());
