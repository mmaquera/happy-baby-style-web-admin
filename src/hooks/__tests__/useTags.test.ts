...
describe('useTags', () => {
  ...
  it('activeTags contains all default tags (all isActive=true)', () => {
    const { result } = renderHook(() => useTags());
    const activeCount = Object.values(result.current.allTags).filter(t => t.isActive).length;
  ...
  describe('createTag', () => {
    it('creates a new custom tag successfully', async () => {
      ...
      let response: Awaited<ReturnType<typeof result.current.createTag>> | undefined;
      await act(async () => {
  ...
  describe('updateTag', () => {
    ...
    it('fails when tag does not exist', async () => {
      ...
      let response: Awaited<ReturnType<typeof result.current.updateTag>> | undefined;
      await act(async () => {
        response = await result.current.updateTag('non-existent-tag', { color: '#fff' });
      });
  ...
  describe('deleteTag', () => {
    it('deletes a custom tag', async () => {
      ...
      let response: Awaited<ReturnType<typeof result.current.deleteTag>> | undefined;
      await act(async () => {
  ...
  it('clearError resets error state', async () => {
    const { result } = renderHook(() => useTags());
    await act(async () => {
      await result.current.createTag('', { color: '#fff', description: 'x', category: 'test', isActive: true });
    });
