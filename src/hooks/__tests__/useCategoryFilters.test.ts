...
describe('useCategoryFilters', () => {
  it('initializes with default state', () => {
    ...
    expect(result.current.sortConfig).toEqual({ field: 'sortOrder', direction: 'asc' });
  ...
  describe('pagination', () => {
    ...
    it('setPaginationFromGraphQL converts to local format', () => {
      const { result } = renderHook(() => useCategoryFilters(0));
      act(() => result.current.setPaginationFromGraphQL({ limit: 20, offset: 40 }));
  ...
  describe('mapFiltersToGraphQL', () => {
    ...
    it('includes search when set', () => {
      const { result } = renderHook(() => useCategoryFilters(0));
      const mapped = result.current.mapFiltersToGraphQL({ search: 'ropa', isActive: true });
