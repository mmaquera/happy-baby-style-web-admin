import { useUpdateCategoryMutation, useUploadSvgMutation } from '@/generated/graphql';
...
export const useUpdateCategory = (): UseUpdateCategoryReturn => {
  const [updateCategoryMutation, { loading: updating, error: updateError }] = useUpdateCategoryMutation({
  ...
  const validateCategoryInput = useCallback((input: UpdateCategoryInput): string[] => {
    const errors: string[] = [];
