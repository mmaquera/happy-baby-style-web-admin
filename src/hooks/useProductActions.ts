...
export const useProductActions = (): UseProductActionsReturn => {
  ...
  // GraphQL hooks
  const { create: createProductMutation, loading: creating } =
    useCreateProduct();
  const { update: updateProductMutation, loading: updating } =
    useUpdateProduct();
  const { remove: deleteProductMutation, loading: deleting } =
    useDeleteProduct();
  const { upload: uploadImageMutation, loading: uploading } =
    useUploadProductImage();
  ...
  const validateProductInput = useCallback(
    (input: CreateProductInput | UpdateProductInput): string[] => {
      const errors: string[] = [];
    ...
    async (input: CreateProductInput): Promise<Product | null> => {
      ...
      try {
        ...
        if (result.data?.createProduct?.data?.entity) {
          ...
          throw new Error(
            'No se pudo crear el producto - respuesta inválida del servidor'
          );
        }
      } catch (err: any) {
    ...
    async (id: string, input: UpdateProductInput): Promise<Product | null> => {
      ...
      try {
        ...
        if (result.data?.updateProduct?.success === false) {
          const serverMessage =
            result.data.updateProduct.message || 'Error del servidor';
          throw new Error(serverMessage);
        }
        ...
        if (result.data?.updateProduct?.data?.entity) {
          ...
          throw new Error(
            'No se pudo actualizar el producto - respuesta inválida del servidor'
          );
        }
      } catch (err: any) {
    ...
    async (id: string, isActive: boolean): Promise<boolean> => {
      ...
      try {
        ...
        if (result.data?.updateProduct?.data?.entity) {
          ...
          throw new Error(
            'No se pudo cambiar el estado del producto - respuesta inválida del servidor'
          );
        }
      } catch (err: any) {
        const errorMessage =
          err.message || 'Error al cambiar el estado del producto';
        setError(errorMessage);
        toast.error(errorMessage);
        return false;
      } finally {
  ...
  const updateProductStock = useCallback(
    async (id: string, quantity: number): Promise<boolean> => {
      ...
      try {
        ...
        const result = await updateProductMutation(id, {
        ...
        if (result.data?.updateProduct?.data?.entity) {
          ...
          throw new Error(
            'No se pudo actualizar el stock del producto - respuesta inválida del servidor'
          );
        }
      } catch (err: any) {
            ...
            switch (operation.operation) {
              ...
              case 'updateCategory':
                if (operation.data && operation.data['categoryId']) {
                  return updateProductMutation(firstId, {
              ...
              case 'updateStock':
                if (
                  operation.data &&
                  operation.data['stockQuantity'] !== undefined
                ) {
                  return updateProductMutation(firstId, {
    ...
    async (file: File, entityId: string): Promise<string | null> => {
      ...
      try {
        ...
        if (
          ...
          result.data?.uploadImage?.data?.url
        ) {
          ...
          const errorMessage =
            result.data?.uploadImage?.message || 'No se pudo subir la imagen';
