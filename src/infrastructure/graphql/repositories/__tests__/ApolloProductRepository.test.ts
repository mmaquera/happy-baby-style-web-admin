...
describe('ApolloProductRepository', () => {
  ...
  beforeEach(() => {
    client = makeClient();
    repo = new ApolloProductRepository(client as unknown as ApolloClient<NormalizedCacheObject>);
  });
  ...
  describe('create', () => {
    ...
    it('returns err when success=false', async () => {
      client.mutate.mockResolvedValue({
        data: { createProduct: { success: false, message: 'SKU duplicado', data: null } },
      });

      const result = await repo.create({ name: 'x', sku: 'dup', price: 10 });
      expect(isErr(result)).toBe(true);
      if (isErr(result)) expect(result.error.message).toContain('SKU duplicado');
    });
  ...
  describe('update', () => {
    ...
    it('returns err when success=false', async () => {
      client.mutate.mockResolvedValue({
        data: { updateProduct: { success: false, message: 'No encontrado', data: null } },
      });
  ...
  describe('delete', () => {
    ...
    it('returns err when success=false', async () => {
      client.mutate.mockResolvedValue({
        data: { deleteProduct: { success: false, message: 'No se puede eliminar' } },
      });
  ...
  describe('uploadImage', () => {
    it('returns url on success', async () => {
      ...
      const result = await repo.uploadImage(new File([''], 'img.jpg'), 'prod-1');
      expect(isOk(result)).toBe(true);
      if (isOk(result)) expect(result.value).toBe('https://cdn.example.com/img.jpg');
    });
