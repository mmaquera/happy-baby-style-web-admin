...
const makeUserDTO = (overrides = {}) => ({
  ...
  profile: { id: 'p-1', firstName: 'Juan', lastName: 'García', phone: null, dateOfBirth: null, avatar: null, fullName: 'Juan García' },
  addresses: [],
  ...overrides,
});
...
describe('ApolloUserRepository', () => {
  ...
  beforeEach(() => {
    client = makeClient();
    repo = new ApolloUserRepository(client as unknown as ApolloClient<NormalizedCacheObject>);
  });
  ...
  describe('findAll', () => {
    it('returns empty page when responseData is null', async () => {
      ...
      if (isOk(result)) expect(result.value).toEqual({ items: [], total: 0, hasMore: false });
    });
  ...
  describe('create', () => {
    it('returns mapped user on success', async () => {
      ...
      const result = await repo.create({ email: 'test@example.com', firstName: 'Juan', lastName: 'García' });
      expect(isOk(result)).toBe(true);
    });

    it('returns err when entity is null', async () => {
      client.mutate.mockResolvedValue({ data: { createUser: { data: null } } });
      const result = await repo.create({ email: 'x@x.com', firstName: 'x', lastName: 'y' });
      expect(isErr(result)).toBe(true);
    });
  ...
  describe('update', () => {
    it('re-fetches full user on success', async () => {
      ...
      client.query.mockResolvedValue({ data: { user: makeUserDTO({ email: 'updated@x.com' }) } });
  ...
  describe('delete', () => {
    it('returns ok(true) on success', async () => {
      client.mutate.mockResolvedValue({ data: { deleteUser: { success: true, message: 'OK' } } });
    ...
    it('returns err when success=false', async () => {
      client.mutate.mockResolvedValue({ data: { deleteUser: { success: false, message: 'Error' } } });
      const result = await repo.delete('u-1');
      expect(isErr(result)).toBe(true);
    });
  });
  ...
  describe('activate', () => {
    it('returns mapped user on success', async () => {
      client.mutate.mockResolvedValue({ data: { activateUser: makeUserDTO({ isActive: true }) } });
  ...
  describe('deactivate', () => {
    it('returns mapped user on success', async () => {
      client.mutate.mockResolvedValue({ data: { deactivateUser: makeUserDTO({ isActive: false }) } });
