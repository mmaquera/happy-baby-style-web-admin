...
describe('ApolloOrderRepository', () => {
  ...
  beforeEach(() => {
    client = makeClient();
    repo = new ApolloOrderRepository(client as unknown as ApolloClient<NormalizedCacheObject>);
  });
  ...
  describe('updateStatus', () => {
    it('re-fetches order after successful mutation', async () => {
      client.mutate.mockResolvedValue({ data: { updateOrderStatus: { id: 'order-1' } } });
      client.query.mockResolvedValue({ data: { order: makeOrderDTO({ status: 'confirmed' }) } });
  ...
  describe('cancel', () => {
    it('re-fetches order after successful cancel', async () => {
      client.mutate.mockResolvedValue({ data: { cancelOrder: { id: 'order-1' } } });
      client.query.mockResolvedValue({ data: { order: makeOrderDTO({ status: 'cancelled' }) } });
