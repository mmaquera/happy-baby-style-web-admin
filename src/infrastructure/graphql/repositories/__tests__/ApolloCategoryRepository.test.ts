...
describe('ApolloCategoryRepository', () => {
  ...
  beforeEach(() => {
    client = makeClient();
    repo = new ApolloCategoryRepository(client as unknown as ApolloClient<NormalizedCacheObject>);
  });
  ...
  describe('create', () => {
    it('returns mapped category on success', async () => {
      ...
      const result = await repo.create({ name: 'Ropa bebé', slug: 'ropa-bebe' });
      expect(isOk(result)).toBe(true);
    });

    it('returns err when success=false', async () => {
      client.mutate.mockResolvedValue({
        data: { createCategory: { success: false, message: 'Slug duplicado', data: null } },
      });
  ...
  describe('update', () => {
    ...
    it('returns err when success=false', async () => {
      client.mutate.mockResolvedValue({
        data: { updateCategory: { success: false, message: 'Error', data: null } },
      });
