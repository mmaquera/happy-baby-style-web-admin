...
describe('LocalTokenStorage', () => {
  ...
  describe('setTokens', () => {
    it('stores access token', () => {
      ...
      expect(localMock.setItem).toHaveBeenCalledWith('accessToken', 'access-123');
    });

    it('stores refresh token when provided', () => {
      storage.setTokens('access-123', 'refresh-456');
      expect(localMock.setItem).toHaveBeenCalledWith('refreshToken', 'refresh-456');
    });
...
describe('SessionTokenStorage', () => {
  ...
  it('setTokens writes to sessionStorage', () => {
    ...
    expect(sessionMock.setItem).toHaveBeenCalledWith('accessToken', 'token-abc');
  });
